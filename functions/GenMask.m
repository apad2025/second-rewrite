function msk = GenMask(img, mag, verboseFLAG, thresh, boneFLAG, noiseSTD)
%GENMASK Generate a binary tissue mask from multi-echo GRE data.
%
%   msk = GenMask(img, mag, verboseFLAG, thresh, boneFLAG, noiseSTD)
%
%   Inputs
%       img         : complex multi-echo image [nx ny nz nTE] (or single volume)
%       mag         : magnitude / weighted magnitude volume [nx ny nz]
%       verboseFLAG : print timing info (default handled by caller)
%       thresh      : intensity threshold as a percentage of the max (e.g. 3)
%       boneFLAG    : if true, attempt to remove bone (marrow) regions
%       noiseSTD    : optional noise standard deviation map to refine the mask
%
%   Output
%       msk         : logical tissue mask [nx ny nz]
%
%   Extracted unchanged from the original DogAnalysis.m pipeline.
%   Depends on: get_echoMIP, and the Image Processing Toolbox
%   (imfill, bwconncomp, activecontour, imerode/imdilate, strel).
