% Function: graphCutIterations_Bipolar_GC
%
...
function [fm,index_fm] = graphCutIterations_Bipolar_GC(imDataParams,algoParams,residual,lmap,cur_ind,DEBUG)
...
for kg = 1:algoParams.NUM_ITERS
    ...
    if rand < prob_bigJump % If we're making a "jumpmin" move.
        ...
        if cur_sign > 0
            stepLocator = (repCurInd+20/dfm>=resLocalMinima) & (resLocalMinima>0);% I think that the first comparison sign here is wrong
            stepLocator = reshape(squeeze(sum(stepLocator,1))+1,sx,sy) ;
            validStep = masksignal>0 & stepLocator<=numMinimaPerVoxel;
        else
            stepLocator = (repCurInd-1/dfm>resLocalMinima) & (resLocalMinima>0);
            stepLocator = reshape(squeeze(sum(stepLocator,1)),sx,sy);
